...
import type { LansiaRecord } from "@/lib/api"

export default function AdminLansiaPage() {
  const [records, setRecords] = useState<LansiaRecord[]>([])
  const [filteredRecords, setFilteredRecords] = useState<LansiaRecord[]>([])
  ...
  const loadRecords = async () => {
    try {
      setIsLoading(true)
      const data = await lansiaApi.getAll()
      setRecords(data.sort((a, b) => new Date(b.tanggal).getTime() - new Date(a.tanggal).getTime()))
    } catch (error) {
      console.error("Error loading Lansia records:", error)
    } finally {
      setIsLoading(false)
    }
  }
    ...
    if (searchQuery) {
      ...
      filtered = filtered.filter(
        (r) =>
          ...
          r.diagnosa_lansia.toLowerCase().includes(query)
      )
    }
    ...
    <div className="admin-page">
      <div className="page-header">
        <div className="page-title">
          <h2>👴 Rekam Pemeriksaan Lansia</h2>
          <p>Total {records.length} rekam pemeriksaan lansia</p>
        </div>
            ...
            {filteredRecords.length === 0 ? (
              <tr>
                <td colSpan={11} className="empty-row">
                  {searchQuery ? "Tidak ada data yang cocok" : "Belum ada rekam Lansia"}
                </td>
              </tr>
            ) : (
              filteredRecords.map((r) => (
                <tr key={r.id}>
                  ...
                  <td className="diagnosa-cell">{r.diagnosa_lansia}</td>
                  <td>{r.td}</td>
                  <td>{r.bb || "-"}</td>
                  <td>{r.gds ? `${r.gds} mg/dL` : "-"}</td>
                  <td>{r.asam_urat ? `${r.asam_urat} mg/dL` : "-"}</td>
                  <td>{r.kolesterol ? `${r.kolesterol} mg/dL` : "-"}</td>
                  <td className="tindakan-cell">{r.tindakan_lansia || "-"}</td>
                  <td>{formatDate(r.jadwal_kontrol_lansia)}</td>
                </tr>
              ))
            )}
          </tbody>
